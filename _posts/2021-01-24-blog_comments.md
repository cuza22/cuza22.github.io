...
## 설치 방법
설치도 매우 쉽다. 
### blog-comments라는 repository를 생성한다. 
여기서 댓글을 한꺼번에 관리하게 된다. 아직까지 내 블로그에 달린 댓글이 없어서 댓글이 달렸을 때 어떻게 보이는지는 잘 모르겠다 ㅎㅎ
### https://utteranc.es/에 접속한다. 
1. Repository에 방금 만든 repository의 이름을 넣고, 

2. Blog Post ⇔ Issue Mapping에서 원하는 방식을 선택한다. 

    utterances는 github issue를 각 포스트와 연결해 댓글처럼 보여주는 것이므로(맞나?), 둘을 연결할 방식을 선택하면 된다. 
